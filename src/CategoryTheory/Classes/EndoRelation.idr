...
class EndoRelationClass (ob: Type) where
  (~>) : EndoRelation_Arrow ob

data EndoRelationRecord : Type where
  MkEndoRelation : 
    (ob: Type) ->
...
recInstance : (rec: EndoRelationRecord) -> EndoRelationClass (recOb rec)
...
instance ObClass EndoRelationRecord where
  Ob = recOb
